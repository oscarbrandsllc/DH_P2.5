  ...
  document.addEventListener('DOMContentLoaded', () => {
    ...
    async function fetchKTCData() {
      ...
      const parseCsvData = async (sheet) => {
        const response = await fetch(`/api/sheet?sheet=${sheet}&id=${GOOGLE_SHEET_ID}`);
    ...
    async function fetchUserAndLeagues(username) {
      const user = await fetchWithCache(`/api/sleeper/user/${username}`);
      ...
      const leagues = await fetchWithCache(`/api/sleeper/user/${state.userId}/leagues/nfl/${currentYear}`);
    ...
    async function ensurePlayerStats(season) {
      ...
      const url = `/api/sleeper/stats/nfl/regular/${season}`;
    ...
    async function analyzeLeague(leagueId) {
      try {
        ...
        const [rosters, users, tradedPicks] = await Promise.all([
          fetchWithCache(`/api/sleeper/league/${leagueId}/rosters`),
          fetchWithCache(`/api/sleeper/league/${leagueId}/users`),
          fetchWithCache(`/api/sleeper/league/${leagueId}/traded_picks`),
        ]);
