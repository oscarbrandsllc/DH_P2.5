...
function showLegend(){ try{ document.getElementById('legend-section')?.classList.remove('hidden'); }catch(e){} }
    ...
    hideLegend();
        ...
        async function fetchDataFromGoogleSheet() {
            ...
            try {
                const [oneQbCsv, sflxCsv] = await Promise.all([
                    fetch(`/api/sheet?sheet=${sheetNames.oneQb}&id=${GOOGLE_SHEET_ID}`).then(res => res.text()),
                    fetch(`/api/sheet?sheet=${sheetNames.sflx}&id=${GOOGLE_SHEET_ID}`).then(res => res.text())
                ]);
        ...
        async function fetchPlayerStatsSheets() {
            ...
            try {
                const seasonPromise = fetch(`/api/sheet?sheet=${PLAYER_STATS_SHEETS.season}&id=${PLAYER_STATS_SHEET_ID}`).then(res => res.text());
                const seasonRanksPromise = fetch(`/api/sheet?sheet=${PLAYER_STATS_SHEETS.seasonRanks}&id=${PLAYER_STATS_SHEET_ID}`).then(res => res.text());
                const weeklyPromises = Object.entries(PLAYER_STATS_SHEETS.weeks).map(async ([week, sheetName]) => {
                    const csv = await fetch(`/api/sheet?sheet=${sheetName}&id=${PLAYER_STATS_SHEET_ID}`).then(res => res.text());
